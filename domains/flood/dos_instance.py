...
warning views and, when the level keeps rising, requests a privileged
sampling-interval change through the act() syscall.

Paths mounted under ``/hydro/shanhu``:

    stations/808J1510/level            m, latest telemetry
    stations/808J1510/sample_interval  s, device configuration
    stations/808J1510/history          list of recent (ts, level) observations
...
class TelemetryStationDriver(Driver):
    ...
    MQTT (see transports in the legacy kernel); here a simulator feeds it."""
    ...
    def normalize(self, raw: object) -> Iterable[tuple[str, object]]:
        frame = dict(raw)
        level = float(frame["level_m"])
        self._history.append((frame.get("ts", time.time()), level))
        yield f"{BASE}/level", level
        yield f"{BASE}/sample_interval", self.sample_interval
        yield f"{BASE}/history", list(self._history)

    # downlink
    def dispatch(self, txn: PendingTxn) -> None:
        self.dispatched.append(txn)
        if txn.action == "set_interval":
            # the device accepts the new config on its next uplink
            self.sample_interval = int(txn.args["interval_s"])

    # fsck rule: a config txn commits when telemetry reports the new interval
...
def spawn_monitor(kernel: Kernel, cap_token: str, sink: Optional[list] = None) -> None:
    ...
    def handler(ctx):
        ...
        if status == "warning" and interval_snap.value != FAST_INTERVAL:
            result = ctx.act(cap_token, f"{BASE}/sample_interval", "set_interval", {"interval_s": FAST_INTERVAL}, expect={f"{BASE}/sample_interval": interval_snap.value})
            events.append(f"act->{result.state}")

    kernel.spawn(ProcessSpec(name="level-monitor", watches=(f"{BASE}/level",), handler=handler, priority=5, description="水位监视：警戒时申请加密采样"))
