...
def run_cnn_v2_forecast(boundary_flow: dict[str, Any],
    ...
    if output_series_path.exists():
        shutil.copy2(output_series_path, target_series_path)
        target_time_steps_path.write_text(
            json.dumps({
                "time_steps_h": time_steps,
                "source": rel(output_series_path),
                "time_series_csv": rel(output_time_series_csv_path) if output_time_series_csv_path.exists() else "",
    ...
    return {
        ...
        "raw_output_series_path": rel(output_series_path) if output_series_path.exists() else "",
        "raw_output_time_series_csv_path": rel(output_time_series_csv_path) if output_time_series_csv_path.exists() else "",
